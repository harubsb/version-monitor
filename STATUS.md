# 📱 バージョン使用期限ステータス
最終更新: **2026-08-17 10:25 JST**（自動(毎日)） ／ 自動更新: 毎日 JST 9:00 頃
...
```diff
- アプリB / Android Target SDK: 現在 33 → 必須 API 35 / 期限 2025-08-31 / 超過351日（申請不可の可能性）
```
...
| React | 18.2.0 | 19.2.3 | — | 🟡 |
| New Architecture | 無効 | 有効 | 将来必須化の見込み（要計画） | 🟡 |
| Android Target SDK | 33 | 35 | 2025-08-31 (超過(351日前)) | ⚫ |
...
## 補足 / 自動取得の信頼性
- Android 期限: スクレイプ失敗(fetch failed) → fallback 値を使用（要手動確認） / source: https://developer.android.com/google/play/requirements/target-sdk
